        ...
        for i in range(inimg_sup.size[0]):
            for j in range(inimg_sup.size[1]):
                if i < inimg_sub.size[0] and j < inimg_sub.size[1]:
                    outpixels[i, j] = encrypt_pixel(inpixels_sup[i, j], inpixels_sub[i, j])
                else:
                    outpixels[i, j] = encrypt_pixel(inpixels_sup[i, j], (255,255,255))
        outimg.save(img3)
        return True
    else:
...
def decrypt_image(img1, img2):
    ...
    for i in range(inimg.size[0]):
        for j in range(inimg.size[1]):
            outpixels[i, j] = decrypt_pixel(inpixels[i, j])
    outimg.save(img2)
    return True
...
if __name__ == '__main__':
    ...
    super_img = raw_input('Container image: ')
    sub_img = raw_input('Secret image: ')
    encrypted_image = super_img.split('.')[-2] + '.encrypted.' + super_img.split('.')[-1]

    print 'Encrypting...'
    if encrypt_image(super_img, sub_img, encrypted_image) and raw_input('Decrypt result? (Y/N): ').upper() == 'Y':
        print 'Decrypting...'
